...
$southwest_charlotte = get_field('southwest_charlotte', 'user_'.$author_id);
...
if( $favHoods != '' ) :
	...
	if(in_array('EASTOVER', $favHoods)) {$hoodLinks['EASTOVER'] = $eastover;}
	...
	if(in_array('MADISON PARK', $favHoods)) {$hoodLinks['MADISON PARK'] = $madison_park;}
	...
	if(in_array('SOUTHWEST CHARLOTTE', $favHoods)) {$hoodLinks['SOUTHWEST CHARLOTTE'] = $southwest_charlotte;}
...
$_28th_row = get_field('28th_row', 'user_'.$author_id);
$_3030_south = get_field('3030_south', 'user_'.$author_id);
$adare_townhomes = get_field('adare_townhomes', 'user_'.$author_id);
$ashbrook_condos = get_field('ashbrook_condos', 'user_'.$author_id);
$beverly_crest_townhomes = get_field('beverly_crest_townhomes', 'user_'.$author_id);
$boxwood = get_field('boxwood', 'user_'.$author_id);
$burning_tree = get_field('burning_tree', 'user_'.$author_id);
$the_celadon_condos = get_field('the_celadon_condos', 'user_'.$author_id);
$chipping_camden = get_field('chipping_camden', 'user_'.$author_id);
$dilworth_heights = get_field('dilworth_heights', 'user_'.$author_id);
$elizabeth_village_condos = get_field('elizabeth_village_condos', 'user_'.$author_id);
$the_garrison_condos = get_field('the_garrison_condos', 'user_'.$author_id);
$grandin_heights = get_field('grandin_heights', 'user_'.$author_id);
$hawthorne_bridge_condos = get_field('hawthorne_bridge_condos', 'user_'.$author_id);
$iveys_townhomes = get_field('iveys_townhomes', 'user_'.$author_id);
$morrison_condos_and_lofts = get_field('morrison_condos_and_lofts', 'user_'.$author_id);
$new_park_west = get_field('new_park_west', 'user_'.$author_id);
$northhampton_place = get_field('northhampton_place', 'user_'.$author_id);
$oak_hill_village = get_field('oak_hill_village', 'user_'.$author_id);
$oakleaf = get_field('oakleaf', 'user_'.$author_id);
$the_oakley = get_field('the_oakley', 'user_'.$author_id);
$olde_georgetown = get_field('olde_georgetown', 'user_'.$author_id);
$park_plaza = get_field('park_plaza', 'user_'.$author_id);
$park_south_station = get_field('park_south_station', 'user_'.$author_id);
$park_west = get_field('park_west', 'user_'.$author_id);
$plaza_vu_condos = get_field('plaza_vu_condos', 'user_'.$author_id);
$preston_flats_condos = get_field('preston_flats_condos', 'user_'.$author_id);
$the_ratcliffe = get_field('the_ratcliffe', 'user_'.$author_id);
$southborough = get_field('southborough', 'user_'.$author_id);
$southgate_on_fairview = get_field('southgate_on_fairview', 'user_'.$author_id);
$springs_village = get_field('springs_village', 'user_'.$author_id);
$staccato_7 = get_field('staccato_7', 'user_'.$author_id);
$terraces_at_quail_hollow = get_field('terraces_at_quail_hollow', 'user_'.$author_id);
$tivoli = get_field('tivoli', 'user_'.$author_id);
$the_trademark = get_field('the_trademark', 'user_'.$author_id);
$woodstone_of_elizabeth = get_field('woodstone_of_elizabeth', 'user_'.$author_id);
...
if( $favComm != '' ) :
	if(in_array('28th Row', $favComm)) {$commLinks['28th Row'] = $_28th_row;}
	if(in_array('3030 South', $favComm)) {$commLinks['3030 South'] = $_3030_south;}
	if(in_array('Adare Townhomes', $favComm)) {$commLinks['Adare Townhomes'] = $adare_townhomes;}
	if(in_array('Ashbrook Condos', $favComm)) {$commLinks['Ashbrook Condos'] = $ashbrook_condos;}
	if(in_array('Beverly Crest Townhomes', $favComm)) {$commLinks['Beverly Crest Townhomes'] = $beverly_crest_townhomes;}
	if(in_array('Boxwood', $favComm)) {$commLinks['Boxwood'] = $boxwood;}
	if(in_array('Burning Tree', $favComm)) {$commLinks['Burning Tree'] = $burning_tree;}
	if(in_array('The Celadon Condos', $favComm)) {$commLinks['The Celadon Condos'] = $the_celadon_condos;}
	if(in_array('Chipping Camden', $favComm)) {$commLinks['Chipping Camden'] = $chipping_camden;}
	if(in_array('Dilworth Heights', $favComm)) {$commLinks['Dilworth Heights'] = $dilworth_heights;}
	if(in_array('Elizabeth Village Condos', $favComm)) {$commLinks['Elizabeth Village Condos'] = $elizabeth_village_condos;}
	if(in_array('The Garrison Condos', $favComm)) {$commLinks['The Garrison Condos'] = $the_garrison_condos;}
	if(in_array('Grandin Heights', $favComm)) {$commLinks['Grandin Heights'] = $grandin_heights;}
	if(in_array('Hawthorne Bridge Condos', $favComm)) {$commLinks['Hawthorne Bridge Condos'] = $hawthorne_bridge_condos;}
	if(in_array('Iveys Townhomes', $favComm)) {$commLinks['Iveys Townhomes'] = $iveys_townhomes;}
	if(in_array('Morrison Condos and Lofts', $favComm)) {$commLinks['Morrison Condos and Lofts'] = $morrison_condos_and_lofts;}
	if(in_array('New Park West', $favComm)) {$commLinks['New Park West'] = $new_park_west;}
	if(in_array('Northhampton Place', $favComm)) {$commLinks['Northhampton Place'] = $northhampton_place;}
	if(in_array('Oak Hill Village', $favComm)) {$commLinks['Oak Hill Village'] = $oak_hill_village;}
	if(in_array('Oakleaf', $favComm)) {$commLinks['Oakleaf'] = $oakleaf;}
	if(in_array('The Oakley', $favComm)) {$commLinks['The Oakley'] = $the_oakley;}
	if(in_array('Olde Georgetown', $favComm)) {$commLinks['Olde Georgetown'] = $olde_georgetown;}
	if(in_array('Park Plaza', $favComm)) {$commLinks['Park Plaza'] = $park_plaza;}
	if(in_array('Park South Station', $favComm)) {$commLinks['Park South Station'] = $park_south_station;}
	if(in_array('Park West', $favComm)) {$commLinks['Park West'] = $park_west;}
	if(in_array('Plaza Vu Condos', $favComm)) {$commLinks['Plaza Vu Condos'] = $plaza_vu_condos;}
	if(in_array('Preston Flats Condos', $favComm)) {$commLinks['Preston Flats Condos'] = $preston_flats_condos;}
	if(in_array('The Ratcliffe', $favComm)) {$commLinks['The Ratcliffe'] = $the_ratcliffe;}
	if(in_array('Southborough', $favComm)) {$commLinks['Southborough'] = $southborough;}
	if(in_array('SouthGate on Fairview', $favComm)) {$commLinks['SouthGate on Fairview'] = $southgate_on_fairview;}
	if(in_array('Springs Village', $favComm)) {$commLinks['Springs Village'] = $springs_village;}
	if(in_array('Staccato 7', $favComm)) {$commLinks['Staccato 7'] = $staccato_7;}
	if(in_array('Terraces at Quail Hollow', $favComm)) {$commLinks['Terraces at Quail Hollow'] = $terraces_at_quail_hollow;}
	if(in_array('Tivoli', $favComm)) {$commLinks['Tivoli'] = $tivoli;}
	if(in_array('The Trademark', $favComm)) {$commLinks['The Trademark'] = $the_trademark;}
	if(in_array('Woodstone of Elizabeth', $favComm)) {$commLinks['Woodstone of Elizabeth'] = $woodstone_of_elizabeth;}
